...
async def start_handler(message: Message):
    ...
    await message.answer(
        ...
        "— статус БПЛА: моментально при появлении официальных данных.",
...
async def uav_handler(callback: CallbackQuery):
    ...
    await callback.message.edit_text(text, reply_markup=back_keyboard(), parse_mode="HTML", disable_web_page_preview=True)
...
async def traffic_handler(callback: CallbackQuery):
    if not await callback_has_access(callback):
        return
    await callback.answer()
    await callback.message.edit_text(format_traffic_text(), reply_markup=back_keyboard(), parse_mode="HTML")
